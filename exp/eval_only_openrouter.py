#!/usr/bin/env python3
"""
Standalone Evaluation Script for SafeDecoding - OpenRouter Version
Evaluates already generated responses using OpenRouter API
"""

import sys
import os
import json
import argparse
import logging
from pathlib import Path

# Add paths
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
from safe_eval_openrouter import DictJudge, GPTJudge_OpenRouter

def get_args():
    parser = argparse.ArgumentParser(description="Evaluate existing SafeDecoding results with OpenRouter")
    
    # Input/Output
    parser.add_argument("--input_file", type=str, required=True, help="Path to generated responses JSON")
    parser.add_argument("--output_file", type=str, default=None, help="Path to save evaluation results")
    
    # Evaluation settings
    parser.add_argument("--judge_model", type=str, default="gemini-2.0-flash", 
                       help="Judge model name (gpt-4o, gemini-2.0-flash, claude-3.5-sonnet, deepseek-r1, etc.)")
    parser.add_argument("--GPT_API", type=str, required=True, help="OpenRouter API key")
    parser.add_argument("--disable_GPT_judge", action="store_true", help="Disable GPT judge, use only DictJudge")
    parser.add_argument("--multi_processing", type=int, default=20, help="Number of parallel processes")
    
    return parser.parse_args()

def main():
    args = get_args()
    
    # Setup logging
    logging.basicConfig(
        level=logging.INFO,
        format='%(asctime)s - %(levelname)s - %(message)s',
        handlers=[logging.StreamHandler()]
    )
    
    # Load input file
    logging.info(f"📂 Loading: {args.input_file}")
    with open(args.input_file, 'r') as f:
        data = json.load(f)
    
    # Extract responses based on file structure
    if isinstance(data, dict) and 'data' in data:
        results = data['data']
        logging.info(f"✅ Loaded {len(results)} responses")
    elif isinstance(data, list):
        results = data
        logging.info(f"✅ Loaded {len(results)} responses")
    else:
        raise ValueError("Unknown input file format")
    
    # Extract goals and responses
    goals = []
    responses = []
    instructions = []
    
    for result in results:
        goal = result.get('goal') or result.get('query') or result.get('instruction')
        response = result.get('output') or result.get('response')
        instruction = result.get('instruction') or goal
        
        if goal and response:
            goals.append(goal)
            responses.append(response)
            instructions.append(instruction)
    
    logging.info(f"📊 Extracted {len(goals)} goal-response pairs")
    
    if len(goals) == 0:
        logging.error("❌ No valid goal-response pairs found!")
        return
    
    # Dictionary Judge
    logging.info("🔍 Running Dictionary Judge...")
    dict_judge = DictJudge()
    scores_dict = dict_judge.eval_batch(responses)
    logging.info(f"✅ Dictionary Judge complete")
    
    # GPT Judge
    scores_gpt = None
    reasons = None
    
    if not args.disable_GPT_judge:
        logging.info(f"🔥 Running GPT Judge with OpenRouter model: {args.judge_model}...")
        gpt_judge = GPTJudge_OpenRouter('gpt', mp=args.multi_processing, 
                                        judge_model=args.judge_model, api=args.GPT_API)
        
        goals_responses_pairs = [(goals[i], responses[i]) for i in range(len(goals))]
        
        scores, reasons = gpt_judge.evaluate(goals_responses_pairs)
        scores_gpt = gpt_judge.score_parse(scores)
        logging.info(f"✅ GPT Judge complete")
    
    # Compile results
    eval_results = []
    for i in range(len(goals)):
        output_formatted = {
            "id": i,
            "goal": goals[i],
            "instruction": instructions[i],
            "output": responses[i],
            "dic_score": scores_dict[i],
            "GPT_score": scores_gpt[i] if scores_gpt else None,
            "GPT_reason": reasons[i] if reasons else None
        }
        eval_results.append(output_formatted)
    
    # Calculate metrics
    gpt_score_sum = 0
    defense_success_count = 0
    
    for result in eval_results:
        if result['GPT_score'] is not None:
            gpt_score_sum += result['GPT_score']
        if result['dic_score'] == 1 or result['dic_score'] == True:
            defense_success_count += 1
    
    harmful_score = gpt_score_sum / len(eval_results) if scores_gpt else 0
    asr = 100 - (defense_success_count / len(eval_results)) * 100
    
    logging.info("")
    logging.info("="*60)
    logging.info("📊 EVALUATION RESULTS")
    logging.info("="*60)
    logging.info(f"Total samples: {len(eval_results)}")
    logging.info(f"Harmful Score: {harmful_score:.2f}")
    logging.info(f"ASR: {asr:.2f}%")
    logging.info(f"Defense Success: {defense_success_count}/{len(eval_results)}")
    logging.info("="*60)
    
    # Save results
    if args.output_file:
        output_path = args.output_file
    else:
        input_path = Path(args.input_file)
        output_path = input_path.parent / f"{input_path.stem}_openrouter_eval.json"
    
    with open(output_path, 'w') as f:
        json.dump(eval_results, f, indent=4)
    
    logging.info(f"💾 Results saved to: {output_path}")
    
    # Save summary
    summary = {
        "total_samples": len(eval_results),
        "harmful_score": harmful_score,
        "asr_percentage": asr,
        "defense_success_count": defense_success_count,
        "judge_model": args.judge_model,
        "api_provider": "OpenRouter",
        "input_file": args.input_file,
        "output_file": str(output_path)
    }
    
    summary_path = Path(output_path).parent / f"{Path(output_path).stem}_summary.json"
    with open(summary_path, 'w') as f:
        json.dump(summary, f, indent=4)
    
    logging.info(f"📈 Summary saved to: {summary_path}")

if __name__ == "__main__":
    main()