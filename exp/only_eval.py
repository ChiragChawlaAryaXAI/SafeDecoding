#!/usr/bin/env python3
"""
Standalone Evaluation Script for SafeDecoding
Evaluates already generated responses without re-running the model
"""
...
def get_args():
    parser = argparse.ArgumentParser(description="Evaluate existing SafeDecoding results")
    
    # Input/Output
    parser.add_argument("--input_file", type=str, required=True, help="Path to generated responses JSON")
    parser.add_argument("--output_file", type=str, default=None, help="Path to save evaluation results")
    
    # Evaluation settings
    parser.add_argument("--judge_model", type=str, default="llama-4-scout", help="Judge model name")
    parser.add_argument("--GPT_API", type=str, required=True, help="Groq API key")
    parser.add_argument("--disable_GPT_judge", action="store_true", help="Disable GPT judge, use only DictJudge")
    parser.add_argument("--multi_processing", type=int, default=20, help="Number of parallel processes")
...
def main():
    ...
    logging.info(f"✅ Dictionary Judge complete")
    ...
    if not args.disable_GPT_judge:
        ...
        gpt_judge = GPTJudge('gpt', mp=args.multi_processing, judge_model=args.judge_model, api=args.GPT_API)
        ...
        logging.info(f"✅ GPT Judge complete")
    ...
    for result in eval_results:
        if result['GPT_score'] is not None:
            gpt_score_sum += result['GPT_score']
        if result['dic_score'] == 1 or result['dic_score'] == True:
    ...
    if args.output_file:
        ...
        output_path = input_path.parent / f"{input_path.stem}_safe_eval.json"
